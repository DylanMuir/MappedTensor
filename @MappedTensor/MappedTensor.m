...
classdef MappedTensor < hgsetget
  ...
  methods
    ...
    % -  Unary minus.
    %
    % Example: m=-MappedTensor(rand(5)); all(m(:) <= 0)
    ...
    %
    % Example: m=-1.*MappedTensor(rand(5)); all(m(:) <= 0)
    ...
    %
    % Example: m=-1*MappedTensor(eye(5)); all(m(:) <= 0)
    ...
    %
    % Example: m=MappedTensor(eye(5))./2; max(m(:))==0.5
    ...
    % +   Plus.
      error('MappedTensor:NotImplemented', ...
        '*** MappedTensor: ''plus'' (A+B) must be performed on a referenced portion of the tensor, or else explicitly with ''arrayfun''.');
    end
